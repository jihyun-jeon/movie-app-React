...
function Coins() {
  ...
  const [coinArr, setCoinArr] = useState<CoinInterface[]>([]);
  //질문1)<CoinInterface[]> :typescript에게 state는 coinarr로 이뤄진 배열이라고 알려주는 것임.
  // coinArr은 배열인데 그 배열안의 요소값의 형식이 이런 형식이라고 인터페이스로 지정해주는 것임.
  // string,obj등은 그냥 초기값에 따라 형식이 정해지지만, 배열은 배열안에 생김새를 알 수 없어 지정해줘야 하는 것임.
